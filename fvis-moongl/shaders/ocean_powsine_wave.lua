...
assert(world.medium.surface.generator == 'powsine_wave', 'shader / wave generator mismatch')
...
local prog, vsh, fsh = gl.make_program({
    vertex="fvis-moongl/shaders/ocean_powsine_wave.vert",
    fragment="fvis-moongl/shaders/ocean_powsine_wave.frag"})
...
loc = {
  ...
  K = gl.get_uniform_location(prog, "K"),
...
M.prepare_scene = function( world, SCR_WIDTH, SCR_HEIGHT )
  ...
  gl.uniformf(loc.K, world.medium.surface.K)
